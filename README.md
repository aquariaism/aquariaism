<img src="https://komarev.com/ghpvc/?username=aquariaism&label=Profile%20views&color=0e75b6&style=flat" />

<p align="center">
  <img src="https://user-images.githubusercontent.com/55389276/140866485-8fb1c876-9a8f-4d6a-98dc-08c4981eaf70.gif" width="320" />
</p>

<h1 align="center">Namaste!, It's Abhishek here</h1>
<h3 align="center">
Computer Science student | Software Developer | Data Scientist | Cricketer
</h3>

---

Hello, I'm a student at Gauhati University,Assam,India.

My journey into Computer Science wasn't something I had completely figured out from the beginning. In fact, I didn't have much idea about the field until my 12th grade. But as I started exploring it, I slowly realized how interesting it is. What fascinates me about Computer Science is how an idea can start as nothing more than a thought and eventually turn into something that millions of people can use. From a few lines of code to systems capable of processing enormous amounts of data, there's something exciting about understanding how all of it works. My main interests currently lie in **AI/ML and Data Science**. I'm learning, experimenting and building projects around these areas while trying to understand not just how things work, but why they work.I'm also really interested in **Web Development** and enjoy building websites and applications. I work with technologies like **React, Node.js, JavaScript, HTML, CSS** and other tools in the web development ecosystem.

I like learning by actually building things, so you'll probably find a mixture of projects, experiments and things I'm currently figuring out in my repositories.
Apart from coding as like every other Indian there's a good chance you'll find me watching or following cricket. What fascinates me the most is the statistical side of the game — the numbers, patterns and everything that happens behind a player's performance and I hope to link my love for cricket, data and tech together one day and make it much more easier so that the game can be more popular globally.

📬 **How to reach me:**  
[abhishek825177@gmail.com](mailto:abhishek825177@gmail.com)
[Linkedin_Abhishek](https://www.linkedin.com/in/abhishek-das-24b24b316/)

---

### 🧰 Tech I work with
<img src="https://skillicons.dev/icons?i=c,cpp,java,html,css,js,python,git,github,vscode,mysql,react" />

---

> Still learning. Still building. Still improving.
